    ...
    <div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div class="flex flex-wrap gap-4 justify-between items-center mb-6">
        ...
        <div class="flex items-center space-x-2 sm:space-x-4">
          ...
          <!-- NUEVO: Enlace a la página de Gestión de Instrumentadores -->
          <router-link to="/instrumentadores" class="bg-white text-gray-700 font-bold py-2 px-4 rounded-lg shadow hover:bg-gray-100 flex items-center space-x-2 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600">
             <UserGroupIcon class="w-5 h-5" />
            <span class="hidden sm:inline">Instrumentadores</span>
          </router-link>
          ...
          <button @click="openNewSurgeryModal" class="bg-blue-600 text-white font-bold py-2 px-4 rounded-lg shadow hover:bg-blue-700 flex items-center space-x-2">
            ...
            <span class="hidden sm:inline">Nueva Cirugía</span>
          </button>
      ...
      <div v-else>
        <!-- MEJORA: Se muestra la tabla solo si hay reportes -->
        <div v-if="reportes.length > 0" class="hidden sm:block bg-white shadow-md rounded-lg overflow-hidden dark:bg-slate-800">
              ...
              <tr v-for="reporte in reportes" :key="reporte.id">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm">
                  <span :class="['px-2 inline-flex text-xs leading-5 font-semibold rounded-full', {
                    'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200': reporte.estado === 'Enviado',
                    'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200': reporte.estado === 'Pendiente',
        ...
        <div v-if="reportes.length > 0" class="sm:hidden space-y-4">
        ...
        <div v-if="reportes.length === 0" class="text-center py-16 px-6 bg-white dark:bg-slate-800 rounded-lg shadow-md">
          <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path vector-effect="non-scaling-stroke" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 13h6m-3-3v6m-9 1V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2z" /></svg>
          <h3 class="mt-2 text-lg font-medium text-gray-900 dark:text-slate-100">No se encontraron reportes</h3>
          <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">No hay reportes que coincidan con los filtros actuales. ¡Intenta crear uno nuevo!</p>
          <div class="mt-6">
            <button @click="openNewSurgeryModal" type="button" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
...
import { UserGroupIcon, ChartBarIcon, PlusIcon, ArrowRightOnRectangleIcon } from '@heroicons/vue/24/outline';
...
const fetchReportes = async () => {
  ...
  try {
    ...
    if (activeFilters.value.statusFilter && activeFilters.value.statusFilter !== 'todos') {
      query.eq('estado', activeFilters.value.statusFilter);
    }
    if (activeFilters.value.searchTerm?.trim()) {
      const search = `%${activeFilters.value.searchTerm.trim()}%`;
      query.or(`paciente.ilike.${search},medico.ilike.${search}`);
    }
    if (activeFilters.value.instrumentador?.trim()) {
      query.ilike('instrumentador_completado', `%${activeFilters.value.instrumentador.trim()}%`);
    }
    if (activeFilters.value.startDate) {
      query.gte('fecha_cirugia', activeFilters.value.startDate);
    }
    if (activeFilters.value.endDate) {
      query.lte('fecha_cirugia', activeFilters.value.endDate);
    }
