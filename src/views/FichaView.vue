    ...
    <div class="w-full max-w-4xl">
      <Transition name="fade" mode="out-in">
        ...
        <div v-if="viewState === 'loading'" class="text-center">
          ...
          <p class="text-lg text-slate-400 mt-4">Buscando información del caso...</p>
        </div>
        ...
        <div v-else-if="viewState === 'error'" class="max-w-lg mx-auto bg-red-500/10 border-l-4 border-red-500 text-red-300 p-4 rounded-md shadow-md flex flex-col items-center space-y-4">
          ...
          <button @click="retryFetch" class="bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-blue-700">
            Reintentar
          </button>
        </div>
        ...
        <SubmissionSuccess 
          ...
          @view-summary="viewState = 'summary_display'"
        />
        
        <SubmittedSummary v-else-if="viewState === 'summary_display'" :reporte="reporte" />
        
        <IdentificationWizard 
          v-else-if="viewState === 'identification'"
          @identification-complete="handleIdentificationComplete"
...
const fetchReporte = async () => {
  ...
  try {
    ...
    if (data.estado === 'Enviado') {
      // Si ya está enviada, mostramos la pantalla de éxito/siguientes pasos.
      viewState.value = 'submitted';
    } else {
      // Si está pendiente, iniciamos el flujo de identificación.
